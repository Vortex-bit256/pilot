...
│   ├── types.ts           # ToolCall, ToolResult, TokenUsage, PermissionMode, ApprovalRequest/Decision
│   ├── events.ts          # AgentEvent: text, text_delta, tool_call, tool_result, usage, done, cancelled
...
| `permissionMode` | `AGENT_MODE` | `"permissionMode"` | `safe` |
...
Команды REPL: `help`, `mode [safe|work|free]`, `exit` (с `/`-префиксом тоже работает).
Пока задача выполняется, **Esc или Ctrl+C** отменяет её: стрим, запущенная команда
и открытый промпт подтверждения прерываются, сессия и история сохраняются —
можно тут же дать следующую задачу.
...
--mode,  -m MODE  режим разрешений: safe (по умолчанию), work, free
--yes,   -y       алиас --mode free: авто-разрешать ВСЕ действия (опасно)
--help,  -h       справка
```

## Режимы разрешений

Каждый инструмент имеет тип (`read`/`write`/`exec`), и режим определяет, какие
вызовы требуют вашего подтверждения (чтение разрешено всегда):

| Режим | write (write_file, edit_file) | exec (run_command) |
|---|---|---|
| `safe` (по умолчанию) | спрашивать | спрашивать |
| `work` | разрешать | спрашивать |
| `free` | разрешать | разрешать ⚠️ |

Перед выполнением агент показывает, что собирается сделать: diff для записи/правки
файлов, команду для `run_command`. Варианты ответа:

- `y` — разрешить этот вызов;
- `n` (или пустой ввод) — отклонить; модель видит отказ и корректирует план;
- `a` — разрешить и **больше не спрашивать про этот инструмент в этой сессии**
  (сессионный allowlist; сбрасывается при выходе).

Переключение на лету — командой `mode` в REPL:

```
> mode          # показать текущий режим
> mode work     # переключиться
```


В неинтерактивном запуске (stdin — не терминал, например пайп) спросить не у кого:
вызовы, требующие подтверждения, автоматически отклоняются. Чтобы разрешить их,
...
3. `Agent.run()` — async-генератор: крутит цикл LLM -> выполнение инструментов -> результат
   ...
   `usage`, `done`/`cancelled`). Ошибка любого инструмента не роняет цикл, а возвращается
   модели как `Error: ...`, чтобы она могла скорректировать действия. Вызовы write/exec-
   инструментов перед выполнением проходят через проверку разрешений (см. выше), а отмена
   задачи (Esc/Ctrl+C) прерывает цикл событием `cancelled` и сохраняет историю валидной.
